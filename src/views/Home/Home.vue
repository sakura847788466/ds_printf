<template>
  <el-tabs v-model="activeName">
    <el-tab-pane label="电子发票"
          ...
          <div class="ul_box"
            ...
            <ul class="el-upload-list el-upload-list--picture">
              <li tabindex="0"
                  ...
                  :key="index"><img v-image-preview
                     :src="(item.type == 'pdf'?img:item.url)"
                     alt=""
                ...
                <a class="el-upload-list__item-name"
                  ...
                  <el-button type="primary"
                             ...
                             @click="print_conver('0012BF7350021B00',item.url,item.type)">打印</el-button>
                </a>
      ...
      <div class="print_box"
           :style="{'display':(istrue?'none':'block'),'padding':'10px'}">
        <img src="./222.png"
             alt=""
             style="display:inline-block;height:100%;width:100%;">
        <div style="display:flex;justify-content:center;">
          <span style="margin:0 auto;">电子发票</span>
    ...
    <el-tab-pane label="出库单"
      ...
      <div class="print_box"
        ...
        <img src="./222.png"
             ...
             style="display:inline-block;height:100%;width:100%;">
        <div style="display:flex;justify-content:center;">
          <span style="margin:0 auto;">出库单</span>
    ...
    <el-tab-pane label="快递单打印"
        ...
        <div class="select_box">
          ...
          <div class="ul_box"
            ...
            <ul class="el-upload-list el-upload-list--picture">
              <li tabindex="0"
                  class="el-upload-list__item is-success"
                  v-for="(item,index) in liList_kuaiDi"
                  :key="index"><img v-image-preview
                     :src="(item.type == 'pdf'?img:item.url)"
                     alt=""
                     class="el-upload-list__item-thumbnail">
      ...
      <div class="print_box"
        ...
        <img src="./222.png"
             ...
             style="display:inline-block;height:100%;width:100%;">
        <div style="display:flex;justify-content:center;">
          <span style="margin:0 auto;">快递单</span>

        </div>
      </div>
    </el-tab-pane>
    <!-- 打印信息 -->
    <el-tab-pane label="打印信息"
                 name="3">
      <div class="print_box"
           :style="{'display':(istrue?'block':'none'),'padding':'10px'}">
  ...
  methods: {
    ...
    uploader_chuKudan ($event) {
      ...
      reader.onload = ($event) => {
        ...
        console.log(this.liList_chuKu)

      }
      reader.readAsDataURL(file);
    },
    uploader_kuaiDi ($event) {
      const file = $event.srcElement.files[0]
      const name = event.target.files[0].name
      const type = event.target.files[0].type.split('/')[1]
      this.index_kuaiDi += 1;
      this.imgName_kuaiDi = name
      let reader = new FileReader();
      reader.onload = ($event) => {
        let result = $event.target.result
        const liItem = { url: result, name: name, key: this.index, file: file, type: type }
        this.liList_kuaiDi.push(liItem)
        console.log(this.liList_kuaiDi)
    ...
    //预览
    open (i) {
      this.$alert('<img src="' + this.liList_chuKu[i].url + '">', {
