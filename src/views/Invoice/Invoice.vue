...
export default {
  ...
  methods: {
    uploader_chuKudan ($event) {
      ...
      const filsMaxsize = 2048;
      const fileSize_n = fileSize / 1024
      const type = event.target.files[0].type.split('/')[1]

      if (fileSize_n > filsMaxsize) {
        this.$message.error('提示', '选择文件不能超过2M')
      } else {
        this.index_chuKu += 1;
        ...
        reader.onload = ($event) => {
          ...
          const liItem = { url: result, name: name, key: this.index_chuKu, file: file, type: type }
